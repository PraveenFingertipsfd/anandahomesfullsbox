...
import getFieldPathOptions from '@salesforce/apex/DynamicFormConfigController.getFieldPathOptions';
...
export default class FormConfiguratorBuilder extends LightningElement {
    ...
    @track contextFields = [];
    // Field-path options for "default value from field" (same object + 1 parent hop)
    @track defaultFieldPathOptions = [];
    ...
    // ── Display view for sections (decorates each field with default-source flags) ──
    get sectionsDisplay() {
        const fieldRefRegex = /^\s*\{!(.+)\}\s*$/;
        const sourceOptions = [
        ...
        return (this.sections || []).map(s => {
            const fields = (s.fields || []).map(f => {
                ...
                const m = dv.match(fieldRefRegex);
                // Explicit source (set by the toggle handler) wins over inference.
                // Inference is only used for fields loaded from saved config that
                // haven't been toggled yet.
                const isFieldRef = f._defaultSource
                    ? f._defaultSource === 'field'
                    : !!m;

                // Search-and-select state for the "From Field" picker.
                const path = m ? m[1] : '';
                const selectedOption = path
                    ? (this.defaultFieldPathOptions || []).find(o => o.value === path)
                    : null;
                const selectedLabel = selectedOption ? selectedOption.label : path;
                const search = f._dfSearch;
                const isSearching = search !== undefined;
                const searchDisplay = isSearching ? search : selectedLabel;
                const filterTerm = (search || '').toLowerCase().trim();
                const allOptions = this.defaultFieldPathOptions || [];
                const filtered = filterTerm
                    ? allOptions.filter(o =>
                        o.label.toLowerCase().includes(filterTerm) ||
                        o.value.toLowerCase().includes(filterTerm))
                    : allOptions;
                // Cap dropdown size so a giant object list doesn't blow up the DOM.
                const visibleOptions = filtered.slice(0, 50);

                return {
                    ...f,
                    defaultSource: isFieldRef ? 'field' : 'literal',
                    defaultIsLiteral: !isFieldRef,
                    defaultIsFieldRef: isFieldRef,
                    defaultPath: path,
                    defaultLiteralValue: isFieldRef ? '' : dv,
                    defaultFieldSearchDisplay: searchDisplay,
                    defaultFieldSelectedLabel: selectedLabel,
                    defaultFieldShowDropdown: !!f._dfOpen,
                    defaultFieldOptions: visibleOptions,
                    defaultFieldHasMatches: visibleOptions.length > 0,
                    defaultFieldHasSelection: !!path
                };
            });
            return {
                ...s,
    ...
    loadFieldsForObject(objectApiName) {
        ...
        // Kick off the field-path options load in parallel so the default-value
        // "From Field" picker is ready by the time the user opens it.
        this.loadDefaultFieldPathOptions(objectApiName);
    ...
    loadDefaultFieldPathOptions(objectApiName) {
        if (!objectApiName) {
            this.defaultFieldPathOptions = [];
            return;
        }
        getFieldPathOptions({ objectApiName })
            .then(result => {
                this.defaultFieldPathOptions = (result || []).map(f => ({
                    label: f.label,
                    value: f.value
                }));
            })
            .catch(() => { this.defaultFieldPathOptions = []; });
    }
    ...
    handleEditConfig(event) {
        ...
        getConfiguration({ configId })
            .then(config => {
                ...
                this.matchingCriteria = config.Matching_Criteria__c || '';
    ...
    handleFieldDefaultSourceChange(event) {
        const sectionId = event.currentTarget.dataset.sectionId;
        const fieldId = event.currentTarget.dataset.fieldId;
        ...
        this.sections = this.sections.map(s => {
            ...
            return this._updateSection(s, {
                fields: s.fields.map(f => {
                    ...
                    // Reset default value when switching source, so we never carry
                    // stale literal text into a field-ref slot or vice-versa.
                    return { ...f, defaultValue: '' , _defaultSource: newSource };
                })
            });
        });
    }

    handleFieldDefaultPathChange(event) {
        const sectionId = event.currentTarget.dataset.sectionId;
        const fieldId = event.currentTarget.dataset.fieldId;
    ...
    // ── Search-and-select handlers for the "From Field" picker ──
    handleDefaultFieldFocus(event) {
        const { sectionId, fieldId } = event.currentTarget.dataset;
        this._patchField(sectionId, fieldId, { _dfOpen: true, _dfSearch: '' });
    }

    handleDefaultFieldSearchInput(event) {
        const { sectionId, fieldId } = event.currentTarget.dataset;
        this._patchField(sectionId, fieldId, {
            _dfSearch: event.target.value || '',
            _dfOpen: true
        });
    }

    handleDefaultFieldBlur(event) {
        const { sectionId, fieldId } = event.currentTarget.dataset;
        // Delay close so an option's mousedown can fire first.
        // eslint-disable-next-line @lwc/lwc/no-async-operation
        setTimeout(() => {
            this._patchField(sectionId, fieldId, { _dfOpen: false, _dfSearch: undefined });
        }, 150);
    }

    handleDefaultFieldSelect(event) {
        const { sectionId, fieldId, value } = event.currentTarget.dataset;
        this.sections = this.sections.map(s => {
            if (s.id !== sectionId) return s;
            return this._updateSection(s, {
                fields: s.fields.map(f =>
    ...
    _patchField(sectionId, fieldId, patch) {
        this.sections = this.sections.map(s => {
            if (s.id !== sectionId) return s;
            return this._updateSection(s, {
                fields: s.fields.map(f =>
