...
import resolveContextDefaults from '@salesforce/apex/DynamicFormExecutionController.resolveContextDefaults';
...
export default class DynamicFormModal extends NavigationMixin(LightningElement) {
    ...
    // Field API names whose default came from a context-record field reference
    // {!path}. Those inputs render disabled — the resolved value just saves on submit.
    @track contextLockedFields = {};
    ...
    connectedCallback() {
        ...
        this.applyContextFieldReferences().then(() => this.loadAllMetadata());
    }
    ...
    /**
     * Resolve "default value from field" references. Any field whose defaultValue
     * looks like `{!OwnerId}` or `{!Project__r.Project_Manager__c}` (and isn't the
     * legacy `{!contextRecordId}` token already handled above) gets its value
     * fetched from the context record and is added to contextLockedFields so the
     * input renders disabled.
     */
    async applyContextFieldReferences() {
        if (!this.formConfig || !this.contextRecordId || !this.formConfig.sections) return;

        const fieldRefRegex = /^\s*\{!(.+)\}\s*$/;
        // path -> list of { field, sectionIdx, fieldIdx } so we can rewrite
        // field.defaultValue once the resolution returns.
        const pathToTargets = {};
        for (const section of this.formConfig.sections) {
            for (const field of (section.fields || [])) {
                const dv = field.defaultValue || '';
                if (dv === '{!contextRecordId}') continue;
                const m = dv.match(fieldRefRegex);
                if (!m) continue;
                const path = m[1].trim();
                if (!path) continue;
                if (!pathToTargets[path]) pathToTargets[path] = [];
                pathToTargets[path].push(field);
            }
        }

        const paths = Object.keys(pathToTargets);
        if (paths.length === 0) return;

        try {
            const resolved = await resolveContextDefaults({
                recordId: this.contextRecordId,
                fieldPaths: paths
            });
            const locks = { ...this.contextLockedFields };
            for (const path of paths) {
                const value = resolved ? resolved[path] : null;
                // Only lock when we got a real value. If the source field on the
                // context record is empty/null, leave the form field editable so
                // the user can fill it manually.
                if (value === undefined || value === null || value === '') {
                    // Clear the {!path} token so it doesn't get treated as a
                    // literal default downstream.
                    for (const f of pathToTargets[path]) f.defaultValue = '';
                    continue;
                }
                for (const f of pathToTargets[path]) {
                    this.fieldValues[f.apiName] = value;
                    locks[f.apiName] = true;
                    // Replace the {!path} token with the resolved value so
                    // buildRenderedSections (which writes field.defaultValue
                    // into fieldValues) doesn't clobber it back to the literal.
                    f.defaultValue = value;
                }
            }
            this.contextLockedFields = locks;
        } catch (e) {
            console.error('resolveContextDefaults failed', e);
        }
    }
        ...
        this.renderedSections = this.formConfig.sections.map((section, idx) => {
            ...
            const fields = (section.fields || []).map(field => {
                ...
                return {
                    ...
                    isDisabled: !!this.contextLockedFields[field.apiName],
                    isContextLocked: !!this.contextLockedFields[field.apiName],
                    referenceObjectName: metadata.referenceObjectName || '',
                    lookupDisplayName: this.lookupDisplayNames[field.apiName] || '',
                    lookupDisplayLabel: this._computeLookupDisplayLabel(field.apiName),
                    lookupPillClass: this.contextLockedFields[field.apiName]
                        ? 'lookup-pill lookup-pill_locked'
                        : 'lookup-pill',
            ...
            fields: section.fields.map(f => {
                if (f.isLookup) {
                    return {
                        ...
                        lookupDisplayLabel: this._computeLookupDisplayLabel(f.apiName),
                        lookupPillClass: this.contextLockedFields[f.apiName]
                            ? 'lookup-pill lookup-pill_locked'
                            : 'lookup-pill',
    ...
    _computeLookupDisplayLabel(apiName) {
        const name = this.lookupDisplayNames[apiName];
        if (name) return name;
        // Name hasn't resolved yet — show a friendly placeholder rather than
        // an empty pill (which looks broken).
        return this.fieldValues[apiName] ? 'Loading…' : '';
    }
