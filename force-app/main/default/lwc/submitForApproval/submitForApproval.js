...
export default class SubmitForApproval extends LightningElement {
    ...
    async loadRequiredFieldsStatus() {
        ...
        try {
            ...
            this.requiredFieldStatuses = (statuses || []).map(f => ({
                ...
                isLookup: f.fieldType === 'reference',
                isStandardInput: f.fieldType !== 'picklist'
                    && f.fieldType !== 'checkbox'
                    && f.fieldType !== 'reference',
                isLookupFilled: f.fieldType === 'reference' && !!f.currentValue,
                lookupDisplayName: f.currentDisplayName || '',
                lookupResults: [],
    ...
    handleRequiredFieldChange(event) {
        ...
        // Update the field status in-memory to reflect the new value. A required checkbox is
        // only satisfied when checked (true); other fields need a non-empty value.
        this.requiredFieldStatuses = this.requiredFieldStatuses.map(f => {
            if (f.fieldApiName === fieldName) {
                const isFilled = f.isCheckbox
                    ? value === true
    ...
    // ── Lookup picker for required-field rows whose fieldType is 'reference' ──
    handleRequiredLookupSearch(event) {
        const fieldName = event.currentTarget.dataset.field;
        const refObject = event.currentTarget.dataset.refobject;
        const term = event.target.value || '';
        if (!refObject || term.trim().length < 2) {
        ...
        searchLookupRecords({
            ...
            .then(results => {
                ...
                this._patchRequiredLookup(fieldName, {
                    lookupResults: list,
                    hasLookupResults: list.length > 0
                });
            })
            .catch(() => {
                this._patchRequiredLookup(fieldName, { lookupResults: [], hasLookupResults: false });
    ...
    handleRequiredLookupSelect(event) {
        const fieldName = event.currentTarget.dataset.field;
        const id = event.currentTarget.dataset.id;
        const name = event.currentTarget.dataset.name || '';
        this.editedFieldValues = { ...this.editedFieldValues, [fieldName]: id };
        this._patchRequiredLookup(fieldName, {
            currentValue: id,
            ...
            lookupResults: [],
            hasLookupResults: false,
            rowClass: 'req-field-edit-row'
        });
    }

    handleRequiredLookupClear(event) {
        const fieldName = event.currentTarget.dataset.field;
        this.editedFieldValues = { ...this.editedFieldValues, [fieldName]: '' };
        this._patchRequiredLookup(fieldName, {
            currentValue: '',
            lookupDisplayName: '',
            isLookupFilled: false,
            isFilled: false,
            lookupResults: [],
            hasLookupResults: false,
            rowClass: 'req-field-edit-row req-field-edit-row-empty'
        });
    }

    _patchRequiredLookup(fieldName, patch) {
        this.requiredFieldStatuses = this.requiredFieldStatuses.map(f =>
            f.fieldApiName === fieldName ? { ...f, ...patch } : f
        );
    }
    ...
    async handleSubmit() {
        ...
        try {
            ...
            if (this.editedFieldValues.Approval_Comments__c) {
                submissionComments = this.editedFieldValues.Approval_Comments__c;
            } else if (commentField && commentField.currentValue) {
                submissionComments = commentField.currentValue;
            }
    ...
    refreshPage() {
        // Refresh LDS-cached record data
        if (this.recordId) {
            try {
                notifyRecordUpdateAvailable([{ recordId: this.recordId }]);
            } catch (e) { /* noop */ }
        }
        // Tell the surrounding record page to refresh related lists / wires
        // (avoid window.location.reload — it re-renders the quick action context
        //  and pops the modal open again).
        try {
