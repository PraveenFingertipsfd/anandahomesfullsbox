...
import getUserNameById from '@salesforce/apex/DynamicApprovalController.getUserNameById';
...
export default class ApprovalStepConfig extends LightningElement {
    ...
    handleUserChange(event) {
        ...
        // Optimistic placeholder while we resolve the real name
        this.updateUserSelection(index, recordId, 'Loading...');

        getUserNameById({ userId: recordId })
            .then(function (name) {
                self.updateUserSelection(index, recordId, name || 'User Selected');
            })
            .catch(function () {
                self.updateUserSelection(index, recordId, 'User Selected');
            });
    }
