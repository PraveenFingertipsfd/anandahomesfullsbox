...
export default class EmailTemplateList extends NavigationMixin(LightningElement) {
    ...
    // Field updates applied to the record when the email is sent
    @track fieldUpdates = [];
    fieldUpdateTypeOptions = [
    ...
    async loadTemplate() {
        try {
            ...
            if (template.fieldUpdatesConfig) {
                this.parseFieldUpdatesConfig(template.fieldUpdatesConfig);
            }
            ...
            if (config.attachments && Array.isArray(config.attachments)) {
                config.attachments.forEach((att, index) => {
                    this.attachments.push({
                        ...
                        documentCategory: att.documentCategory || '',
    ...
    // ============ FIELD UPDATES ON SEND ============

    parseFieldUpdatesConfig(configJson) {
        try {
            const arr = JSON.parse(configJson);
            if (Array.isArray(arr)) {
                this.fieldUpdates = arr.map((r, i) => ({
                    id: this.generateId(),
                    index: i,
                    field: r.field || '',
                    type: r.type || 'Text',
            ...
            console.error('Error parsing field updates config:', e);
        }
    }

    buildFieldUpdatesConfig() {
        const rows = this.fieldUpdates
            .filter((r) => r.field && r.field.trim())
            .map((r) => ({ field: r.field.trim(), type: r.type || 'Text', value: r.value }));
    ...
    addFieldUpdateRow() {
        this.fieldUpdates = [
            ...this.fieldUpdates,
            { id: this.generateId(), index: this.fieldUpdates.length, field: '', type: 'DateTime', value: 'NOW' }
        ];
    }

    handleFieldUpdateChange(event) {
        const idx = parseInt(event.target.dataset.index, 10);
        const key = event.target.dataset.key;
        const val = event.target.value;
        this.fieldUpdates = this.fieldUpdates.map((r, i) => (i === idx ? { ...r, [key]: val } : r));
    }

    handleFieldUpdateDelete(event) {
        const idx = parseInt(event.target.dataset.index, 10);
        this.fieldUpdates = this.fieldUpdates
            .filter((_, i) => i !== idx)
            .map((r, i) => ({ ...r, index: i }));
    }

    get hasFieldUpdates() {
        return this.fieldUpdates.length > 0;
    }
    ...
    buildTemplateData() {
        return {
            ...
            fieldUpdatesConfig: this.buildFieldUpdatesConfig(),
