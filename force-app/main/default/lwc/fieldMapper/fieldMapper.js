...
import getMappingConfiguration from '@salesforce/apex/FieldMapperController.getMappingConfiguration';
...
export default class FieldMapper extends LightningElement {
    ...
    get formTitle() {
        return this.editingConfigId
            ? 'Edit Field Mapping Configuration'
    ...
    handleEditConfig(event) {
        ...
        this.isLoadingList = true;
        getMappingConfiguration({ configId })
            .then(cfg => {
                this.resetForm();
                this.editingConfigId = cfg.id;
                this.mappingName = cfg.name;
                this.fromObject = cfg.fromObject;
                this.toObject = cfg.toObject;
                this.isActive = cfg.isActive;
                this.mappingTrigger = cfg.mappingTrigger || 'Insert and Update';
                this.showForm = true;

                const p1 = getFieldList({ objectApiName: cfg.fromObject }).then(result => {
                    this.fromObjectFields = result.map(f => ({
                        label: f.label + ' (' + f.apiName + ')', value: f.apiName, type: f.type
                    }));
                    this.showFromFields = true;
                });
                const p2 = getFieldList({ objectApiName: cfg.toObject }).then(result => {
                    this.toObjectFields = result.map(f => ({
                        label: f.label + ' (' + f.apiName + ')', value: f.apiName, type: f.type
                    }));
                    this.showToFields = true;
                });
                return Promise.all([p1, p2]).then(() => cfg);
            })
            .then(cfg => {
                const rows = (cfg.details || []).map((d, i) => ({
                    id: Date.now() + i,
                    fromField: d.fromField,
                    toField: d.toField,
                    fromFieldType: d.fromFieldType,
                    toFieldType: d.toFieldType,
                    compatibleToFields: []
                }));
                const compLoads = rows.map(row =>
                    getCompatibleFields({ objectApiName: this.toObject, fieldType: row.fromFieldType })
                        .then(result => {
                            row.compatibleToFields = result.map(f => ({
                                label: f.label + ' (' + f.apiName + ')', value: f.apiName, type: f.type
                            }));
                        })
                        .catch(() => {})
                );
                return Promise.all(compLoads).then(() => { this.fieldMappings = rows; });
            })
            .catch(error => {
