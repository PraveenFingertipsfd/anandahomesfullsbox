...
export default class EmailSender extends LightningElement {
    ...
    // Document(s) pinned when the composer is opened from the Document Viewer's
    ...
    // Opens the composer with no template selected — executive writes everything by hand.
    async handleStartCustomEmail() {
        try {
            this.isLoading = true;
            this.showModal = true;
            this.currentStep = 1;
    ...
    // Opens the composer from the Document Viewer's "Send Email" action with the
    // chosen document pinned as an attachment. The template dropdown still lists the
    // object's email config templates so the user can pick one or write their own.
    @api
    async openForDocument(documentTemplateId, documentName) {
        try {
            this.isLoading = true;
            this.showModal = true;
            this.currentStep = 1;
            ...
            if (documentTemplateId) {
                this.pinnedAttachments = [this.buildDocAttachment(documentTemplateId, documentName)];
            }
            this.selectedTemplateId = 'custom';
            this.isCustomEmail = true;
            await this.loadBlankEmail();
    ...
    buildDocAttachment(documentTemplateId, documentName) {
        ...
        return {
            id: 'doc_' + documentTemplateId,
            name: name,
            fileName: name + '.pdf',
            type: 'VFPage',
            vfPageName: 'DynamicDocumentPDF',
    ...
    mapPreparedData(data) {
        ...
        // Re-attach any pinned documents (from the Document Viewer's Send Email action)
        // so switching templates never drops the document the user opened the composer for.
        if (this.pinnedAttachments && this.pinnedAttachments.length) {
            const existingIds = new Set(this.attachments.map(a => a.id));
            this.pinnedAttachments.forEach(p => {
                if (!existingIds.has(p.id)) this.attachments.push({ ...p });
            });
        }
    ...
    handlePickManagerFile(event) {
        const docId = event.currentTarget.dataset.id;
        const name = event.currentTarget.dataset.name;
        if (this.uploadedFiles.some(f => f.documentId === docId)) {
            this.showToast('Already added', name + ' is already attached', 'warning');
            return;
        }
        this.uploadedFiles = [...this.uploadedFiles, { documentId: docId, name: name }];
    ...
    async loadDesignerDocs() {
        try {
            const docs = await getDesignerDocuments({
                recordId: this.recordId,
                objectApiName: this._objectApiName
            });
            this.designerDocs = (docs || []).map(d => ({ id: d.id, name: d.name }));
        } catch (error) {
            console.error('Error loading designer documents:', error);
            this.designerDocs = [];
        }
    }

    handlePickDesignerDoc(event) {
        const tid = event.currentTarget.dataset.id;
        const name = event.currentTarget.dataset.name;
        const attId = 'doc_' + tid;
