...
export default class DocumentDesigner extends LightningElement {
    ...
    // Lookup-parent field options (e.g. Booking__r.Total_Flat_Cost__c) for the table
    // "Parent Field" total picker, loaded when a table section is being edited.
    @track tableLookupParentFields = [];
    ...
    // Header / Footer
    @track headerConfig = {
        left: { type: 'none', content: '', logoUrl: '', logoContentVersionId: '', logoSize: 100 },
        center: { type: 'none', content: '', logoUrl: '', logoContentVersionId: '', logoSize: 100 },
        right: { type: 'none', content: '', logoUrl: '', logoContentVersionId: '', logoSize: 100 },
    ...
    get mergeFieldList() {
        ...
        this.fieldOptions.forEach(f => {
            // Normal value token (use this when you want the plain/currency value).
            out.push({ label: f.label, value: '{!' + f.value + '}', type: f.type });
            // "In words" token for EVERY field, e.g. {!INWORDS(Amount__c)}. You pick which to
            // use; the same field is therefore available both as a value and in words.
            out.push({
                label: f.label + ' (in words)',
                value: '{!INWORDS(' + f.value + ')}',
                type: f.type
            });
        });
        return out;
    ...
    // Convert a lookup field API name to its relationship name (e.g. Account__c -> Account__r,
    // OwnerId -> Owner) for building {!Rel.Field} merge paths.
    _toRelationshipName(lookupFieldApi) {
        let relName = lookupFieldApi;
    ...
    get parentMergeFieldList() {
        ...
        const relName = this._toRelationshipName(this.selectedLookupField);
        const out = [];
        this.parentFieldOptions.forEach(f => {
            out.push({ label: f.label, value: '{!' + relName + '.' + f.value + '}', type: f.type });
            // "In words" token for every parent field too.
            out.push({
                label: f.label + ' (in words)',
                value: '{!INWORDS(' + relName + '.' + f.value + ')}',
                type: f.type
            });
        });
        return out;
    ...
    // Inline style for the preview img: pin width to the configured logoSize.
    _headerLogoStyle(col) {
        const size = parseInt(col && col.logoSize, 10);
        const px = Number.isFinite(size) && size > 0 ? size : 100;
        return `width:${px}px; max-width:${px}px; max-height:none; height:auto;`;
    }
    get headerLeftLogoStyle()   { return this._headerLogoStyle(this.headerLeft); }
    get headerCenterLogoStyle() { return this._headerLogoStyle(this.headerCenter); }
    get headerRightLogoStyle()  { return this._headerLogoStyle(this.headerRight); }
    ...
    _buildPreviewSection(s, i) {
        ...
        if (type === 'table') {
            ...
            const aggLabels = {
                ...
                min: 'Min', max: 'Max', parent: 'Parent value'
            };
            const hasAggregate = sec._tableColumns.some(c => c.total && c.total !== 'none');
            sec._showTotalsRow = (s.showTotals === true) || hasAggregate;
            sec._totalsRowLabel = s.totalsLabel || 'Total';
            // When row numbers are shown the label sits in the "#" cell, otherwise it goes
            // into the first column that has no aggregate (matching the renderer).
            let labelPlaced = sec.showRowNumbers === true;
            sec._totalsRow = sec._tableColumns.map(col => {
                const agg = col.total || 'none';
                let value = '';
                if (agg !== 'none') {
                    value = aggLabels[agg] || '';
                } else if (!labelPlaced) {
                    value = sec._totalsRowLabel;
                    labelPlaced = true;
                }
                return { _key: col._key + '-tot', value };
            });
        }
    ...
    handleEditSection(event) {
        ...
        this.tableColumns = section.columns
            ? section.columns.map((c) => ({
                ...c,
                _id: c._id || this._generateId(),
                _isParentTotal: c.total === 'parent'
            }))
            : [];
    ...
    handleSaveSection() {
        ...
        if (section.type === 'table') {
            section.columns = this.tableColumns.map(({ _id, _isParentTotal, ...rest }) => rest);
        }
        if (section.fields) {
    ...
    handleAddColumn() {
        this.tableColumns = [...this.tableColumns, {
            label: '', field: '', total: 'none', totalParentField: '',
            _isParentTotal: false, _id: this._generateId()
        }];
    }
    ...
    handleColumnChange(event) {
        ...
        this.tableColumns = this.tableColumns.map(c => {
            ...
            updated._isParentTotal = updated.total === 'parent';
            return updated;
        });
    }

    get columnTotalOptions() {
        return [
            { label: 'No Total', value: 'none' },
            { label: 'Sum', value: 'sum' },
            { label: 'Count', value: 'count' },
            { label: 'Average', value: 'average' },
            { label: 'Min', value: 'min' },
            { label: 'Max', value: 'max' },
            { label: 'Parent Field', value: 'parent' }
        ];
    }

    get parentFieldOptionsForCombobox() {
        return [
            { label: '-- Select Parent Field --', value: '' },
            // Main-record fields (the table's immediate parent).
            ...this.fieldOptions.map(f => ({ label: f.label + ' (' + f.value + ')', value: f.value })),
            // Related (lookup) parent fields, e.g. Booking__r.Total_Flat_Cost__c.
    ...
    _loadTableLookupParentFields() {
        ...
        const requests = lookups.map(lk =>
            getParentFields({ objectApiName: this.templateData.objectApiName, lookupFieldName: lk.value })
                .then(result => {
                    const relName = this._toRelationshipName(lk.value);
                    return (result || []).map(pf => ({
                        label: lk.label + ' > ' + pf.label + ' (' + relName + '.' + pf.value + ')',
                        value: relName + '.' + pf.value
                    }));
                })
                .catch(() => [])
        );
        Promise.all(requests).then(groups => {
            this.tableLookupParentFields = groups.reduce((all, g) => all.concat(g), []);
        });
    }
    ...
    handleInsertMergeField(event) {
        ...
        if (this.mergeFieldTarget === 'headerLeft' || this.mergeFieldTarget === 'headerCenter' || this.mergeFieldTarget === 'headerRight') {
            ...
            // Insert as its own paragraph so it gets normal paragraph spacing. Appending a bare
            // token after the existing HTML lands it outside any <p>, which renders as a loose
            // line with no gap above it. Wrapping in <p> keeps spacing consistent.
            this.editingSection = {
                ...this.editingSection,
                content: (this.editingSection.content || '') + '<p>' + mergeField + '</p>'
            };
    ...
    _getDefaultHeaderCol() {
        return { type: 'none', content: '', logoUrl: '', logoContentVersionId: '', logoSize: 100 };
    }
